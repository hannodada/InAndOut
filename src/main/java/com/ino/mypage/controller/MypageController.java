...
public class MypageController {
	...
	public String mypage(Model model) {
		...
		// 판매글, 갤러리 갯수 세기
		int countsales = service.countsales("user001");
		model.addAttribute("countsales", countsales);
		int countgallery = service.countgallery("user001");
	...
	public String riderPage(Model model, HttpSession session) {
		...
		String starid = dto.getUser_id();
		logger.info("아이디값 :" + starid);
		if (starid.equals(user_id)) {
			int avg = service.mystar(starid);
			logger.info("평균점수 :" + avg);
			model.addAttribute("avg", avg);
		} else {
			logger.info("!!starid가 세션에 로그인한 값과 다릅니다");
		}
		// 라이더 extra 불러오기
...
//	}
	...
	public String riderSetting(Model model,@RequestParam HashMap<String,
		...
		logger.info("id값 : "+id);
		MypageDTO riderSetting = service.riderSetting(id);
