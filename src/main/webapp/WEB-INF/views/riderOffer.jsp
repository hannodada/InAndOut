...
<script>
		...
		function myroutlistPrint(list,idx){
			var content='';
			content +='<h2 class="tag_top">'+'총 '+list.length+'건의 배송 제안 요청이 있어요.'+'</h2>';
		    ...
		    content += '<div>배송제안번호 : ' + item.delivery_offer_no + '</div>';
		    content += '<div>IN ' + item.from_addr + '</div>';
		    content += '<div>OUT ' + item.to_addr + '</div>';
		    content += '<div class="button-box">';
		    content += '<button id="recognize" onclick="location.href=\'deliveryState.do?idx=' + item.delivery_offer_no + '\'">승인</button>';
